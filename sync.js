...
window.MTMSync = (() => {
  ...
  async function forgetDeletedAccount(userId){
    switching=true;clearTimeout(syncTimer);
    try{
      ...
      await clearAccountContent();await rawDelete("accountVaults",userId);
  ...
  return {forgetDeletedAccount,restoreRemote,reloadAfterRestore,switchHousehold,isSwitching:()=>switching,build:BUILD,onLocalPut,onLocalDelete,syncNow,queueExisting,state,saveState,api,resolveConflict,rawAll,clearTemporaryShareData,activateAccount,signOutAccount,removeCurrentHouseholdData,initializeAccountIsolation,defaultServer:defaultProductionServer};
})();
...
async function renderSyncCenter(){
  ...
  if(s.token&&!s.reauthRequired&&!s.user){try{const account=await sync.api("/v1/account");s={...s,user:account.user};await sync.saveState(s);}catch{s=await sync.state();}}
  ...
  if(!signedIn){$("#signInForm").onsubmit=async event=>{event.preventDefault();try{const d=await sync.api("/v1/auth/login",{method:"POST",body:JSON.stringify({email:$("#syncEmail").value,password:$("#syncPassword").value})});await sync.activateAccount(d);await acceptPendingInvitation(sync);renderSyncCenter();}catch(e){alert(e.message);}};
    ...
    $("#reviewAccountDeletion").onclick=()=>reviewAccountDeletion(sync);
...
async function reviewAccountDeletion(sync){
  let plan;
  try{plan=await sync.api('/v1/account/deletion');}catch(e){return alert(e.status===404?'Account deletion needs the server update. Please try again after it is installed.':e.message);}
  modalBody.innerHTML=`<h2>Delete your account</h2><p>${esc(plan.email)}</p><p>${esc(plan.notice)}</p><h3>Households to delete</h3><ul>${plan.households.map(h=>`<li>${esc(h.name)} — ${h.records} records</li>`).join('')||'<li>None</li>'}</ul>${plan.blockers.map(x=>`<p class="banner">${esc(x)}</p>`).join('')}${plan.canDelete?'<div id="deleteAccountForm"><label for="deletePassword">Current password</label><input id="deletePassword" type="password" autocomplete="current-password" required><label for="deleteWord">Type DELETE</label><input id="deleteWord" autocomplete="off" required pattern="DELETE"><p>This cannot be undone. This device’s saved copy for this account will also be removed.</p><button class="btn" type="button">Permanently delete my account</button></div>':'<p>Support can help resolve these items before deletion.</p><button id="requestAccountDeletion" class="btn secondary" type="button">Request deletion help</button>'}<p id="deleteNotice" role="status"></p><button id="closeDeleteAccount" class="btn secondary" type="button">Go back</button>`;
  modal.showModal();
  document.getElementById('closeDeleteAccount').onclick=()=>modal.close();
  const requestButton=document.getElementById('requestAccountDeletion'),requestId=crypto.randomUUID();
  if(requestButton)requestButton.onclick=async()=>{requestButton.disabled=true;try{await sync.api('/v1/support',{method:'POST',body:JSON.stringify({requestId,subject:'Account deletion request',topic:'account',message:'I want to delete my account. Please help resolve: '+plan.blockers.join(' ')})});document.getElementById('deleteNotice').textContent='Your deletion request is in Help and support. Your account remains available while support reviews it.';}catch(e){document.getElementById('deleteNotice').textContent=e.message;requestButton.disabled=false;}};
  const form=document.getElementById('deleteAccountForm');
  if(form)form.querySelector('button').onclick=async event=>{event.preventDefault();if(![...form.querySelectorAll('input')].every(input=>input.reportValidity()))return;const button=form.querySelector('button');button.disabled=true;
    try{await sync.api('/v1/account/deletion',{method:'POST',body:JSON.stringify({password:document.getElementById('deletePassword').value,confirm:document.getElementById('deleteWord').value,confirmation:plan.confirmation})});await sync.forgetDeletedAccount(plan.userId);modal.close();await renderSyncCenter();alert('Your account was deleted.');}
    catch(e){document.getElementById('deleteNotice').textContent=e.message;button.disabled=false;}
  };
}
