...
async function renderSyncCenter(){
  ...
  <div class="card"><h3>Server</h3><div class="field"><label>Server address</label><input id="syncServer" value="${esc(s.serverUrl||"http://127.0.0.1:8788")}" placeholder="http://192.168.1.20:8788"></div><button id="saveServer" class="btn secondary">Save address</button><p class="hint">${esc(status)}</p></div>
  ...
  if(!signedIn){$("#loginSync").onclick=async()=>{try{await sync.saveState({...await sync.state(),serverUrl:$("#syncServer").value.trim()});const d=await sync.api("/v1/auth/login",{method:"POST",body:JSON.stringify({email:$("#syncEmail").value,password:$("#syncPassword").value})});await sync.saveState({...await sync.state(),token:d.token,lastError:null});try{const memberships=await sync.api("/v1/households");if(memberships.households?.length){await sync.saveState({...await sync.state(),householdId:memberships.households[0].id,cursor:0});await sync.syncNow();}}catch{}renderSyncCenter();}catch(e){alert(e.message);}};
  ...
  }else{try{const d=await sync.api("/v1/households"),area=$("#householdArea");
    const memberships=Array.isArray(d.households)?d.households:[];
    let current=await sync.state();
    let active=memberships.find(h=>h.id===current.householdId)||memberships[0]||null;
    if(active&&current.householdId!==active.id){current={...current,householdId:active.id,cursor:0,lastError:null};await sync.saveState(current);}
    area.innerHTML=memberships.length?`<div class="field"><label>Active household</label><select id="activeHousehold">${memberships.map(h=>`<option value="${h.id}" ${h.id===active?.id?"selected":""}>${esc(h.name)} — ${esc(h.role)}</option>`).join("")}</select></div><div class="btn-row"><button id="createInvite" class="btn secondary">Create caregiver invite</button><button id="joinInvite" class="btn secondary">Join with invitation</button></div>`:`<div class="banner">You are signed in, but this account is not connected to a household yet.</div><div class="btn-row"><button id="joinInvite" class="btn secondary">Join with invitation</button></div>`;
    if($("#activeHousehold"))$("#activeHousehold").onchange=async e=>{await sync.saveState({...await sync.state(),householdId:e.target.value,cursor:0,lastError:null});await sync.syncNow();renderSyncCenter();};
    if($("#createInvite"))$("#createInvite").onclick=async()=>{try{const role=confirm("OK creates a caregiver invitation. Cancel creates a read-only viewer invitation.")?"caregiver":"viewer",d=await sync.api("/v1/invitations",{method:"POST",body:JSON.stringify({householdId:$("#activeHousehold").value,role})});const text=`More than Measured invitation code: ${d.code}
Expires: ${d.expiresAt}`;if(navigator.share){try{await navigator.share({title:"More than Measured invitation",text});return;}catch(e){if(e?.name!=="AbortError")console.warn(e);}}try{await navigator.clipboard.writeText(d.code);alert(`Invitation code copied to the clipboard:
${d.code}

Expires ${d.expiresAt}`);}catch{prompt(`Invitation expires ${d.expiresAt}. Copy this code:`,d.code);}}catch(e){alert(e.message);}};
    $("#joinInvite").onclick=async()=>{const code=prompt("Invitation code:");if(!code)return;try{const d=await sync.api("/v1/invitations/join",{method:"POST",body:JSON.stringify({code})});await sync.saveState({...await sync.state(),householdId:d.householdId,cursor:0,lastError:null});await sync.syncNow();alert("Joined household successfully. Household sync is now active on this device.");renderSyncCenter();}catch(e){alert(e.message);}};
    if(active)await sync.syncNow();
  }catch(e){$("#householdArea").innerHTML=`<div class="banner">${esc(e.message)}</div>`;}
    $("#syncNow").onclick=async()=>{await sync.syncNow();renderSyncCenter();};$("#prepareData").onclick=async()=>{if(!confirm("Create a safety checkpoint and add copies of all existing local family data to this household? Nothing local will be removed."))return;try{const count=await sync.queueExisting();alert(`${count} existing records are ready to synchronize.`);await sync.syncNow();renderSyncCenter();}catch(e){alert(e.message);}};$("#logoutSync").onclick=async()=>{try{await sync.api("/v1/auth/logout",{method:"POST",body:"{}"});}catch{}await sync.saveState({id:"current",serverUrl:s.serverUrl,cursor:0});renderSyncCenter();};}
