...
const APP = { name: "More than Measured", version: "0.8.14", schemaVersion: 3 };
...
const LIFEPRINT_SIGN_SLUGS={
  ...
  "Playground":"play","Aquarium":"fish","Blocks":"block","Teddy Bear":"bear","Tablet":"computer",
...
function openSpeechGuide(kind){
  ...
  if(kind==="signs"){modalBody.innerHTML=`<h2>🖐️ ASL Quick Guide</h2><p class="hint">Choose a useful everyday word or phrase to open its Lifeprint ASL instruction page. Labels such as “All Done” are matched to the intended ASL concept rather than automatically fingerspelled. A word may appear in more than one section because families use it in different situations.</p><div class="sign-groups">${QUICK_SIGN_GROUPS.map(([group,signs])=>`<section><h3>${group}</h3><div class="sign-grid">${signs.map((sign)=>`<button class="small-action quick-sign" data-url="${esc(lifeprintSignUrl(sign))}">${esc(sign)}</button>`).join("")}</div></section>`).join("")}</div><p class="hint">Demonstrations open on Lifeprint/ASL University and require an internet connection.</p>`;modal.showModal();document.querySelectorAll(".quick-sign").forEach((b)=>b.onclick=()=>open(b.dataset.url,"_blank","noopener"));return;}
