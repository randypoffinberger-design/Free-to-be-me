...
const APP={name:'More than Measured',version:'0.3.2',schemaVersion:1};
...
const DEFAULT_VOCAB_CATEGORIES=['Uncategorized','Animals','Toys','Body Parts','Food & Drink','People','Actions','Places','Clothing','Vehicles','Social Words','Descriptive Words','Other'];
async function getVocabCategories(){const saved=await getSetting('vocabCategories',[]),all=['Uncategorized',...saved,...DEFAULT_VOCAB_CATEGORIES];return [...new Set(all.map(x=>String(x).trim()).filter(Boolean))];}
const capabilityValue=(item,key)=>item[key]===undefined?key==='speak':Boolean(item[key]);
const abilityDate=(item,key)=>item[`${key}Date`]||(key==='speak'?item.date||'':'');
const entryDate=item=>[abilityDate(item,'speak'),abilityDate(item,'identify'),abilityDate(item,'asl')].filter(Boolean).sort()[0]||item.date||'';
const firstSaidDate=item=>abilityDate(item,'speak')||item.date||'';
const languagesText=languages=>(Array.isArray(languages)?languages:[]).map(x=>`${x.language}: ${x.word}`).join('\n');
const wordCategories=item=>[item.category,...(Array.isArray(item.additionalCategories)?item.additionalCategories:[])].map(x=>String(x||'').trim()).filter((x,i,a)=>x&&a.indexOf(x)===i).slice(0,3);
function parseLanguages(text){return String(text||'').split(/\r?\n/).map(x=>x.trim()).filter(Boolean).map(line=>{const parts=line.split(/\s*(?::|\s[-–—]\s)\s*/,2);return{language:(parts.length>1?parts[0]:'Other').trim(),word:(parts.length>1?parts[1]:parts[0]).trim()};}).filter(x=>x.word);}
async function normalizeVocabulary(words){for(const item of words){let changed=false;if(!item.category){item.category='Uncategorized';changed=true;}if(!Array.isArray(item.additionalCategories)){item.additionalCategories=[];changed=true;}else{const clean=item.additionalCategories.filter(c=>c&&c!==item.category).filter((c,i,a)=>a.indexOf(c)===i).slice(0,2);if(clean.join('|')!==item.additionalCategories.join('|')){item.additionalCategories=clean;changed=true;}}for(const key of ['speak','identify','asl'])if(item[key]===undefined){item[key]=key==='speak';changed=true;}if(item.speak&&!item.speakDate&&item.date){item.speakDate=item.date;changed=true;}if(!Array.isArray(item.languages)){item.languages=[];changed=true;}if(changed)await put('words',item);}return words;}

async function renderVocabulary(){
  const profiles=await getAll('profiles'),words=await normalizeVocabulary(await getAll('words')),categories=await getVocabCategories();
  if(!profiles.length){view.innerHTML=`<div class="empty card"><div class="big">🗣️</div><h2>Create a child profile first</h2><p>Speech and language entries are connected to a child so every word remains part of the correct story.</p><button id="vocabCreateProfile" class="btn">Create profile</button></div>`;$('#vocabCreateProfile').onclick=openProfileForm;return;}
  const years=[...new Set(words.flatMap(x=>['speak','identify','asl'].map(k=>abilityDate(x,k))).map(x=>String(x||'').slice(0,4)).filter(Boolean))].sort((a,b)=>b-a);
  ...
  const refresh=()=>{
    const profile=$('#vocabProfile').value,q=wordKey($('#vocabSearch').value),category=$('#vocabCategory').value,year=$('#vocabYear').value,month=$('#vocabMonth').value,exact=$('#vocabDate').value,sort=$('#vocabSort').value,selected=['speak','identify','asl'].filter(k=>$('#filter'+(k==='asl'?'Asl':k[0].toUpperCase()+k.slice(1))).checked);
    const totals=words.filter(x=>profile==='all'||x.profileId===profile);$('#totalWords').textContent=totals.length;$('#totalSpeak').textContent=totals.filter(x=>capabilityValue(x,'speak')).length;$('#totalIdentify').textContent=totals.filter(x=>capabilityValue(x,'identify')).length;$('#totalAsl').textContent=totals.filter(x=>capabilityValue(x,'asl')).length;
    let shown=words.filter(x=>{const dates=selected.filter(k=>capabilityValue(x,k)).map(k=>abilityDate(x,k)).filter(Boolean),assigned=wordCategories(x);return(profile==='all'||x.profileId===profile)&&(!category||assigned.includes(category))&&selected.some(k=>capabilityValue(x,k))&&(!q||wordKey(`${x.word} ${x.notes||''} ${assigned.join(' ')} ${languagesText(x.languages)}`).includes(q))&&(!exact||dates.includes(exact))&&(!exact&&!year||dates.some(d=>d.startsWith(year)))&&(!exact&&!month||dates.some(d=>d.slice(5,7)===month));});
    shown.sort(sort==='alpha'?(a,b)=>a.word.localeCompare(b.word,undefined,{sensitivity:'base'}):sort==='oldest'?(a,b)=>entryDate(a).localeCompare(entryDate(b))||a.word.localeCompare(b.word):(a,b)=>entryDate(b).localeCompare(entryDate(a))||a.word.localeCompare(b.word));
    $('#vocabSummary').textContent=`${shown.length} of ${words.length} ${words.length===1?'entry':'entries'}`;
    const drawCard=x=>`<details class="word-card card"><summary><strong>${esc(x.word)}</strong>${entryDate(x)?`<span>${fmtDate(entryDate(x))}</span>`:''}</summary><div class="word-details"><div class="word-detail-meta">${wordCategories(x).map(c=>`<span class="category-chip">${esc(c)}</span>`).join('')}<span>${esc(names[x.profileId]||'Child')}</span></div>${x.notes?`<p class="word-notes">${esc(x.notes)}</p>`:''}<div class="ability-checks">${[['speak','Speak'],['identify','Identify'],['asl','ASL']].map(([k,label])=>`<div class="ability-row"><label><input type="checkbox" class="ability-toggle" data-id="${x.id}" data-key="${k}" ${capabilityValue(x,k)?'checked':''}> ${label}</label><input type="date" class="ability-date" data-id="${x.id}" data-key="${k}" value="${abilityDate(x,k)}" aria-label="${label} learned date"></div>`).join('')}</div>${x.languages.length?`<div class="language-list"><strong>Additional languages</strong>${x.languages.map(l=>`<span>${esc(l.language)}: ${esc(l.word)}</span>`).join('')}</div>`:''}<div class="word-actions"><button class="small-action edit-word" data-id="${x.id}" type="button">Edit</button><button class="small-action danger-link delete-word" data-id="${x.id}" type="button">Delete</button></div></div></details>`;
    if(sort==='category'&&shown.length){const groups=new Map();for(const item of shown)for(const assigned of wordCategories(item).filter(c=>!category||c===category)){if(!groups.has(assigned))groups.set(assigned,[]);groups.get(assigned).push(item);}$('#vocabList').innerHTML=[...groups].sort(([a],[b])=>a.localeCompare(b,undefined,{sensitivity:'base'})).map(([name,items])=>`<section class="category-group"><h2>${esc(name)}</h2>${items.sort((a,b)=>firstSaidDate(b).localeCompare(firstSaidDate(a))||a.word.localeCompare(b.word,undefined,{sensitivity:'base'})).map(drawCard).join('')}</section>`).join('');}else $('#vocabList').innerHTML=shown.length?shown.map(drawCard).join(''):`<div class="empty card"><div class="big">🔎</div><p>No speech or language entries match these filters.</p></div>`;
    document.querySelectorAll('.ability-toggle').forEach(box=>box.onchange=async()=>{const item=words.find(x=>x.id===box.dataset.id);if(!item)return;item[box.dataset.key]=box.checked;if(box.checked&&!item[`${box.dataset.key}Date`])item[`${box.dataset.key}Date`]=isoToday();item.date=entryDate(item)||item.date;item.updatedAt=nowISO();await put('words',item);refresh();});
    document.querySelectorAll('.ability-date').forEach(input=>input.onchange=async()=>{const item=words.find(x=>x.id===input.dataset.id);if(!item)return;item[`${input.dataset.key}Date`]=input.value;if(input.value)item[input.dataset.key]=true;item.date=entryDate(item)||item.date;item.updatedAt=nowISO();await put('words',item);refresh();});
    document.querySelectorAll('.edit-word').forEach(b=>b.onclick=()=>openWordForm(profiles,words.find(x=>x.id===b.dataset.id),categories));
    document.querySelectorAll('.delete-word').forEach(b=>b.onclick=async()=>{const item=words.find(x=>x.id===b.dataset.id);if(item&&confirm(`Delete “${item.word}”? This cannot be undone from this screen.`)){await createSnapshot(`Before deleting speech/language entry ${item.word}`);await deleteItem('words',item.id);renderVocabulary();}});
  };
  ['vocabProfile','vocabSearch','vocabCategory','vocabSort','vocabYear','vocabMonth','vocabDate','filterSpeak','filterIdentify','filterAsl'].forEach(id=>$('#'+id).addEventListener(id==='vocabSearch'?'input':'change',refresh));
  $('#clearVocabFilters').onclick=()=>{['vocabCategory','vocabYear','vocabMonth','vocabDate','vocabSearch'].forEach(id=>$('#'+id).value='');$('#vocabProfile').value='all';$('#vocabSort').value='alpha';['filterSpeak','filterIdentify','filterAsl'].forEach(id=>$('#'+id).checked=true);refresh();};
  $('#addWord').onclick=()=>openWordForm(profiles,null,categories);
  $('#bulkWords').onclick=()=>openBulkVocabulary(profiles,words,categories);
  $('#manageCategories').onclick=()=>openCategoryManager(categories);
  refresh();
}

function openWordForm(profiles,item=null,categories=[]){
  const assigned=wordCategories(item||{}),categoryOptions=(selected,optional=false)=>`${optional?'<option value="">None</option>':''}${categories.map(c=>`<option ${selected===c?'selected':''}>${esc(c)}</option>`).join('')}`;
  modalBody.innerHTML=`<h2>${item?'Edit':'Add'} speech/language entry</h2><div class="form-grid"><div class="field"><label>Child</label><select id="wordProfile">${profiles.map(p=>`<option value="${p.id}" ${item?.profileId===p.id?'selected':''}>${esc(p.name)}</option>`).join('')}</select></div><div class="field"><label>Word or phrase</label><input id="wordText" value="${esc(item?.word||'')}" autocomplete="off"></div><div class="field"><label>Primary category</label><div class="inline-field"><select id="wordCategory">${categoryOptions(assigned[0]||'Uncategorized')}</select><button id="quickCategory" class="btn secondary" type="button">New</button></div></div><div class="field"><label>Secondary category <span class="hint">(optional)</span></label><select id="wordCategory2">${categoryOptions(assigned[1],true)}</select></div><div class="field"><label>Tertiary category <span class="hint">(optional)</span></label><select id="wordCategory3">${categoryOptions(assigned[2],true)}</select></div><fieldset class="ability-field dated-abilities"><legend>Abilities and dates learned</legend>${[['speak','Speak'],['identify','Identify'],['asl','ASL']].map(([k,label])=>`<div class="ability-row"><label><input id="word${k==='asl'?'Asl':k[0].toUpperCase()+k.slice(1)}" type="checkbox" ${item?capabilityValue(item,k)?'checked':'':k==='speak'?'checked':''}> ${label}</label><input id="word${k==='asl'?'Asl':k[0].toUpperCase()+k.slice(1)}Date" type="date" value="${item?abilityDate(item,k):k==='speak'?isoToday():''}" aria-label="${label} learned date"></div>`).join('')}</fieldset><div class="field"><label>Additional spoken languages <span class="hint">(optional, one per line)</span></label><textarea id="wordLanguages" placeholder="Spanish: gato&#10;French: chat">${esc(languagesText(item?.languages))}</textarea><span class="hint">Use Language: word or phrase. Dates are not required.</span></div><div class="field"><label>Notes <span class="hint">(optional)</span></label><textarea id="wordNotes">${esc(item?.notes||'')}</textarea></div><button id="saveWord" class="btn full" type="button">Save entry</button></div>`;
  if(!modal.open)modal.showModal();
  $('#quickCategory').onclick=async()=>{const name=prompt('New category name:')?.trim();if(!name)return;if(categories.some(c=>wordKey(c)===wordKey(name)))return alert('That category already exists.');categories.push(name);await setSetting('vocabCategories',categories);for(const id of ['wordCategory','wordCategory2','wordCategory3']){const option=document.createElement('option');option.textContent=name;if(id==='wordCategory')option.selected=true;$('#'+id).append(option);}};
  for(const key of ['Speak','Identify','Asl']){const box=$('#word'+key),date=$('#word'+key+'Date');box.onchange=()=>{if(box.checked&&!date.value)date.value=isoToday();};date.onchange=()=>{if(date.value)box.checked=true;};}
  $('#saveWord').onclick=async()=>{const word=$('#wordText').value.trim(),profileId=$('#wordProfile').value;if(!word)return alert('Please enter a word or phrase.');const all=await getAll('words');if(all.some(x=>x.id!==item?.id&&x.profileId===profileId&&wordKey(x.word)===wordKey(word)))return alert('That word or phrase is already listed for this child.');const category=$('#wordCategory').value||'Uncategorized',additionalCategories=[$('#wordCategory2').value,$('#wordCategory3').value].filter(c=>c&&c!==category).filter((c,i,a)=>a.indexOf(c)===i);const record={id:item?.id||uid(),profileId,word,category,additionalCategories,speak:$('#wordSpeak').checked,identify:$('#wordIdentify').checked,asl:$('#wordAsl').checked,speakDate:$('#wordSpeakDate').value,identifyDate:$('#wordIdentifyDate').value,aslDate:$('#wordAslDate').value,languages:parseLanguages($('#wordLanguages').value),notes:$('#wordNotes').value.trim(),createdAt:item?.createdAt||nowISO(),updatedAt:nowISO(),syncStatus:'local'};record.date=entryDate(record)||item?.date||isoToday();await put('words',record);modal.close();renderVocabulary();};
}
...
function openBulkVocabulary(profiles,existing,categories){
  ...
  if(!modal.open)modal.showModal();$('#previewBulk').onclick=()=>{const profileId=$('#bulkProfile').value,category=$('#bulkCategory').value,additionalCategories=[$('#bulkCategory2').value,$('#bulkCategory3').value].filter(c=>c&&c!==category).filter((c,i,a)=>a.indexOf(c)===i),speak=$('#bulkSpeak').checked,identify=$('#bulkIdentify').checked,asl=$('#bulkAsl').checked,parsed=parseBulkVocabulary($('#bulkText').value,$('#bulkFallback').value);const seen=new Set(existing.filter(x=>x.profileId===profileId).map(x=>wordKey(x.word))),fresh=[];let duplicates=0;for(const entry of parsed.entries){const key=wordKey(entry.word);if(seen.has(key)){duplicates++;continue;}seen.add(key);fresh.push(entry);}modalBody.innerHTML=`<h2>Review import</h2><p><strong>${fresh.length}</strong> ready • ${duplicates} duplicate${duplicates===1?'':'s'} skipped • ${parsed.skipped.length} unread line${parsed.skipped.length===1?'':'s'}</p><p class="hint">Categories: ${[category,...additionalCategories].map(esc).join(', ')} • ${[speak?'Speak':'',identify?'Identify':'',asl?'ASL':''].filter(Boolean).join(', ')||'No abilities selected'}</p><div class="import-preview">${fresh.map(x=>`<div class="preview-row"><strong>${esc(x.word)}</strong><span>${fmtDate(x.date)}</span></div>`).join('')||'<p>No new entries were found.</p>'}</div><div class="btn-row"><button id="backBulk" class="btn secondary" type="button">Go back</button>${fresh.length?'<button id="importBulk" class="btn" type="button">Import reviewed entries</button>':''}</div>`;$('#backBulk').onclick=()=>openBulkVocabulary(profiles,existing,categories);if(fresh.length)$('#importBulk').onclick=async()=>{await createSnapshot('Before speech/language bulk import');for(const x of fresh)await put('words',{id:uid(),profileId,word:x.word,date:x.date,category,additionalCategories,speak,identify,asl,speakDate:speak?x.date:'',identifyDate:identify?x.date:'',aslDate:asl?x.date:'',languages:[],notes:'',createdAt:nowISO(),updatedAt:nowISO(),syncStatus:'local'});modal.close();alert(`${fresh.length} ${fresh.length===1?'entry':'entries'} imported.`);renderVocabulary();};};
}

function openCategoryManager(categories){
  const draw=()=>{modalBody.innerHTML=`<h2>Manage categories</h2><div class="category-list">${categories.map(c=>`<div class="category-row"><span>${esc(c)}</span><div>${c!=='Uncategorized'?`<button class="small-action rename-category" data-name="${esc(c)}" type="button">Rename</button><button class="small-action danger-link remove-category" data-name="${esc(c)}" type="button">Delete</button>`:''}</div></div>`).join('')}</div><button id="addCategory" class="btn full" type="button" style="margin-top:14px">Add category</button>`;
    $('#addCategory').onclick=async()=>{const name=prompt('New category name:')?.trim();if(!name)return;if(categories.some(c=>wordKey(c)===wordKey(name)))return alert('That category already exists.');categories.push(name);await setSetting('vocabCategories',categories);draw();};
    document.querySelectorAll('.rename-category').forEach(b=>b.onclick=async()=>{const old=b.dataset.name,name=prompt('Rename category:',old)?.trim();if(!name||name===old)return;if(categories.some(c=>wordKey(c)===wordKey(name)))return alert('That category already exists.');const words=await getAll('words');for(const item of words.filter(x=>wordCategories(x).includes(old))){if(item.category===old)item.category=name;item.additionalCategories=(item.additionalCategories||[]).map(c=>c===old?name:c).filter(c=>c!==item.category);item.updatedAt=nowISO();await put('words',item);}categories[categories.indexOf(old)]=name;await setSetting('vocabCategories',categories);renderVocabulary();draw();});
    document.querySelectorAll('.remove-category').forEach(b=>b.onclick=async()=>{const name=b.dataset.name;if(!confirm(`Delete “${name}”? It will be removed from every assigned entry.`))return;await createSnapshot(`Before deleting vocabulary category ${name}`);const words=await getAll('words');for(const item of words.filter(x=>wordCategories(x).includes(name))){if(item.category===name){const replacement=(item.additionalCategories||[]).find(c=>c!==name)||'Uncategorized';item.category=replacement;}item.additionalCategories=(item.additionalCategories||[]).filter(c=>c!==name&&c!==item.category);item.updatedAt=nowISO();await put('words',item);}categories=categories.filter(c=>c!==name);await setSetting('vocabCategories',categories);renderVocabulary();draw();});
  };
...
async function exportBackup(){const b=await collectBackup(),stamp=new Date().toISOString().replace(/[:.]/g,'-');downloadBlob(new Blob([JSON.stringify(b,null,2)],{type:'application/json'}),`MoreThanMeasured-Backup-${stamp}.ftbmbackup`);await setSetting('lastBackupAt',b.exportedAt);renderBackup();}
...
function validateBackup(b){if(!b||b.format!=='ftbm-backup'||!b.data||typeof b.schemaVersion!=='number')throw new Error('This is not a valid More than Measured backup.');if(b.schemaVersion>APP.schemaVersion)throw new Error('This backup was created by a newer version of the app.');for(const s of STORE_NAMES.filter(x=>x!=='snapshots'))if(!Array.isArray(b.data[s]||[]))throw new Error(`Backup section ${s} is invalid.`);}
