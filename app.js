...
const APP = { name: "More than Measured Test", version: "0.10.0-amazon-products-test", schemaVersion: 5 };
...
function openAmazonProductCatalog(category,title,safetyNote=""){
  const products=AMAZON_PRODUCT_CATALOG.filter(([categories])=>categories.split(" ").includes(category));
  const draw=(query="")=>{
    ...
    $("#amazonProductList").innerHTML=matches.length?matches.map(([,name,url])=>`<a class="education-link" href="${esc(url)}" target="_blank" rel="noopener noreferrer"><strong>${esc(name)}</strong><span>Check the current listing, seller, size, age range, price, return terms, and safety information.</span><small>Open on Amazon ↗</small></a>`).join(""):`<div class="empty"><p>No products match that search.</p></div>`;
  };
  modalBody.innerHTML=`<h2>🛍️ ${esc(title)}</h2><p>These caregiver-supplied links are shopping options, not endorsements. Choose around the child’s actual goal, age, size, motor skills, sensory preferences, cleaning needs, supervision, and product hazards.</p>${safetyNote?`<div class="banner">${safetyNote}</div>`:""}<div class="banner"><strong>Amazon link notice:</strong> These are standard product links. More than Measured does not currently earn a commission from purchases. Products, sellers, prices, and availability can change.</div><div class="field"><label>Search these products</label><input id="amazonProductSearch" type="search" placeholder="Headphones, books, swing…"></div><p id="amazonProductCount" class="hint"></p><div id="amazonProductList" class="education-links"></div>`;
  modal.showModal();draw();
  $("#amazonProductSearch").oninput=(event)=>draw(event.target.value);
}

function openProductGuide(title,items,category){
  if(category){openAmazonProductCatalog(category,`${title} products`);return;}
