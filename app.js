...
const APP={name:'More than Measured',version:'0.3.1',schemaVersion:1};
...
async function renderVocabulary(){
  ...
  <div class="vocab-controls card">
    ...
    <div class="field"><label>Sort</label><select id="vocabSort"><option value="alpha">Alphabetical</option><option value="newest">Date added — newest</option><option value="oldest">Date added — oldest</option></select></div>
  ...
  const refresh=()=>{
    ...
    let shown=words.filter(x=>{const dates=selected.filter(k=>capabilityValue(x,k)).map(k=>abilityDate(x,k)).filter(Boolean);return(profile==='all'||x.profileId===profile)&&(!category||x.category===category)&&selected.some(k=>capabilityValue(x,k))&&(!q||wordKey(`${x.word} ${x.notes||''} ${x.category||''} ${languagesText(x.languages)}`).includes(q))&&(!exact||dates.includes(exact))&&(!exact&&!year||dates.some(d=>d.startsWith(year)))&&(!exact&&!month||dates.some(d=>d.slice(5,7)===month));});
    ...
    $('#vocabList').innerHTML=shown.length?shown.map(x=>`<details class="word-card card"><summary><strong>${esc(x.word)}</strong>${entryDate(x)?`<span>${fmtDate(entryDate(x))}</span>`:''}</summary><div class="word-details"><div class="word-detail-meta"><span class="category-chip">${esc(x.category)}</span><span>${esc(names[x.profileId]||'Child')}</span></div>${x.notes?`<p class="word-notes">${esc(x.notes)}</p>`:''}<div class="ability-checks">${[['speak','Speak'],['identify','Identify'],['asl','ASL']].map(([k,label])=>`<div class="ability-row"><label><input type="checkbox" class="ability-toggle" data-id="${x.id}" data-key="${k}" ${capabilityValue(x,k)?'checked':''}> ${label}</label><input type="date" class="ability-date" data-id="${x.id}" data-key="${k}" value="${abilityDate(x,k)}" aria-label="${label} learned date"></div>`).join('')}</div>${x.languages.length?`<div class="language-list"><strong>Additional languages</strong>${x.languages.map(l=>`<span>${esc(l.language)}: ${esc(l.word)}</span>`).join('')}</div>`:''}<div class="word-actions"><button class="small-action edit-word" data-id="${x.id}" type="button">Edit</button><button class="small-action danger-link delete-word" data-id="${x.id}" type="button">Delete</button></div></div></details>`).join(''):`<div class="empty card"><div class="big">🔎</div><p>No speech or language entries match these filters.</p></div>`;
...
function openCategoryManager(categories){
  const draw=()=>{modalBody.innerHTML=`<h2>Manage categories</h2><div class="category-list">${categories.map(c=>`<div class="category-row"><span>${esc(c)}</span><div>${c!=='Uncategorized'?`<button class="small-action rename-category" data-name="${esc(c)}" type="button">Rename</button><button class="small-action danger-link remove-category" data-name="${esc(c)}" type="button">Delete</button>`:''}</div></div>`).join('')}</div><button id="addCategory" class="btn full" type="button" style="margin-top:14px">Add category</button>`;
    ...
    document.querySelectorAll('.rename-category').forEach(b=>b.onclick=async()=>{const old=b.dataset.name,name=prompt('Rename category:',old)?.trim();if(!name||name===old)return;if(categories.some(c=>wordKey(c)===wordKey(name)))return alert('That category already exists.');const words=await getAll('words');for(const item of words.filter(x=>x.category===old)){item.category=name;item.updatedAt=nowISO();await put('words',item);}categories[categories.indexOf(old)]=name;await setSetting('vocabCategories',categories);renderVocabulary();draw();});
    document.querySelectorAll('.remove-category').forEach(b=>b.onclick=async()=>{const name=b.dataset.name;if(!confirm(`Delete “${name}”? Entries in it will move to Uncategorized.`))return;await createSnapshot(`Before deleting vocabulary category ${name}`);const words=await getAll('words');for(const item of words.filter(x=>x.category===name)){item.category='Uncategorized';item.updatedAt=nowISO();await put('words',item);}categories=categories.filter(c=>c!==name);await setSetting('vocabCategories',categories);renderVocabulary();draw();});
  };
