...
const QUICK_SIGNS=["more","all done","help","eat","drink","bathroom","hurt","stop","wait","yes","no","mom","dad","play","sleep","please","thank you"];
function openSpeechGuide(kind){
  ...
  if(kind==="signs"){modalBody.innerHTML=`<h2>🖐️ ASL Quick Guide</h2><p class="hint">Each item opens an outside demonstration search. Future native-app clips can be added to the prepared media field without changing the list structure.</p><div class="sign-grid">${QUICK_SIGNS.map((sign)=>`<button class="small-action quick-sign" data-sign="${sign}">${sign}</button>`).join("")}</div>`;modal.showModal();document.querySelectorAll(".quick-sign").forEach((b)=>b.onclick=()=>open(`https://www.signingsavvy.com/search/${encodeURIComponent(b.dataset.sign)}`,"_blank","noopener"));return;}
...
async function renderFoodDiary() {
  ...
  const draw = () => { $("#foodList").innerHTML = entries.length ? [...entries].sort((a,b)=>a.name.localeCompare(b.name)).map((x) => `<div class="list-item"><div><strong>${x.kind === "meal" ? "🍱" : "🍎"} ${esc(x.name)}</strong><div class="hint">${labels[x.category]}${x.date ? ` • ${fmtDate(x.date)}` : ""}</div>${x.notes ? `<p>${esc(x.notes)}</p>` : ""}</div><div><button class="small-action edit-food" data-id="${x.id}">Edit</button><button class="small-action danger-link delete-food" data-id="${x.id}">Delete</button></div></div>`).join("") : `<div class="empty card"><p>No foods or meals added yet.</p></div>`;
    document.querySelectorAll(".edit-food").forEach((b) => b.onclick = async () => { const x = entries.find((e)=>e.id===b.dataset.id), name = prompt("Edit name", x.name); if (name === null || !name.trim()) return; const notes = prompt("Edit notes", x.notes || ""); if (notes === null) return; x.name=name.trim(); x.notes=notes.trim(); x.updatedAt=nowISO(); await persist(); draw(); });
  ...
  $("#foodProfile").onchange = async (e) => { profileId=e.target.value; await load(); };
  $("#saveFood").onclick = async () => { const name=$("#foodName").value.trim(); if(!name) return alert("Enter a food or meal name."); entries.push({id:uid(),kind:$("#foodKind").value,name,category:$("#foodCategory").value,date:$("#foodDate").value,notes:$("#foodNotes").value.trim(),createdAt:nowISO(),updatedAt:nowISO()}); await persist(); $("#foodName").value=""; $("#foodNotes").value=""; draw(); };
...
async function renderHealthWellness(){
  ...
  $("#mthfrGuide").onclick=()=>openInfoGuide("🧬 MTHFR without the internet panic",`<p>MTHFR is a gene involved in processing folate. C677T and A1298C are common variants; having one does not automatically cause disease, autism, high homocysteine, medication problems, or a need for methylated vitamins.</p><ul><li>Testing uses blood or saliva, but major medical sources do not recommend common-variant testing for most people.</li><li>If homocysteine is high, a clinician looks at nutrition, B-vitamin status, thyroid or kidney disease, medicines, and other causes. Treatment often does not depend on the common MTHFR result.</li><li>People with common variants can process folic acid. Do not replace or megadose folate based only on a direct-to-consumer result.</li><li>A common MTHFR variant is not, by itself, a vaccine contraindication and does not predict a universal reaction to OTC medicine. Review each medication, supplement, allergy, and medical condition with the prescriber or pharmacist.</li></ul><div class="education-links"><a class="education-link" href="https://medlineplus.gov/lab-tests/mthfr-gene-test/" target="_blank" rel="noopener"><strong>MedlinePlus MTHFR test guide</strong><span>When testing may be used and how results are interpreted.</span><small>Open source ↗</small></a><a class="education-link" href="https://www.cdc.gov/folic-acid/data-research/mthfr/index.html" target="_blank" rel="noopener"><strong>CDC MTHFR and folic acid facts</strong><span>Common variants and folate processing.</span><small>Open source ↗</small></a></div>`);
