...
const ANALYTICS_FEATURES = Object.freeze({
  ...
  sleepWhy: "sleep-sanctuary",
  sleepExample: "sleep-sanctuary",
  sleepRoutine: "sleep-sanctuary",
  sleepEnvironment: "sleep-sanctuary",
  sleepFamilyRoutine: "sleep-sanctuary",
  sleepCompare: "sleep-sanctuary",
  sleepMagnesium: "sleep-sanctuary",
  sleepBeds: "sleep-sanctuary",
  sleepPreferences: "sleep-sanctuary",
  sleepProducts: "sleep-sanctuary",

  fun: "asd-friendly-fun",
  funAccessPass: "asd-friendly-fun",
  funPlaces: "asd-friendly-fun",
  funSocial: "asd-friendly-fun",
  funExplore: "asd-friendly-fun",
  funCruising: "asd-friendly-fun",
  funFilms: "asd-friendly-fun",
...
const VILLAGE_LOCAL_INDEX = [
  ...
  ["sensory-play", "Free Sensory & Inclusive Play", "Sensory", "Free sensory games, inclusive activities, and printables with access options.", "sensoryPlay", "games play eye gaze switch matching sensory printables"],
  ["book-library", "Build Your Child’s Library", "Skill Building", "Explore books curated by More Than Measured for children and families.", "bookLibrary", "books reading library stories"],
  ["skills-home", "Skill Building", "Learning and skills", "Open potty training, visual schedules, social stories, routines, and daily skill resources.", "skills", "learning teaching practice routine independence books reading library"],
...
const routes = {
  ...
  skills: renderSkills,
  bookLibrary: renderBookLibrary,
  sensoryPlay: renderSensoryPlay,
  ...
  sleepWhy: () => renderSleepSanctuary("sleepWhy"),
  sleepExample: () => renderSleepSanctuary("sleepExample"),
  sleepRoutine: () => renderSleepSanctuary("sleepRoutine"),
  sleepEnvironment: () => renderSleepSanctuary("sleepEnvironment"),
  sleepFamilyRoutine: () => renderSleepSanctuary("sleepFamilyRoutine"),
  sleepCompare: () => renderSleepSanctuary("sleepCompare"),
  sleepMagnesium: () => renderSleepSanctuary("sleepMagnesium"),
  sleepBeds: () => renderSleepSanctuary("sleepBeds"),
  sleepPreferences: () => renderSleepSanctuary("sleepPreferences"),
  sleepProducts: () => renderSleepSanctuary("sleepProducts"),

  health: renderHealthWellness,
  sensory: renderSensorySupport,
  fun: renderAsdFriendlyFunExpanded,
  funAccessPass: () => renderAsdFriendlyFunExpanded("funAccessPass"),
  funPlaces: () => renderAsdFriendlyFunExpanded("funPlaces"),
  funSocial: () => renderAsdFriendlyFunExpanded("funSocial"),
  funExplore: () => renderAsdFriendlyFunExpanded("funExplore"),
  funCruising: () => renderAsdFriendlyFunExpanded("funCruising"),
  funFilms: () => renderAsdFriendlyFunExpanded("funFilms"),
  ...
  products: renderProductDirectory,
...
async function performNavigation(r, options = {}) {
  if (window.MTMLayouts?.isSaving()) return;
  window.MTMLayouts?.dispose();
  let route;
  if (options.back) {
...
async function refreshVisibleRouteFromSync(){
  if(!remoteRefreshPending||remoteRefreshRunning||modal.open||window.MTMLayouts?.isEditing())return;
...
function moduleActions() {
  return {
    ...Object.fromEntries(window.MTMModules.ids.filter(id => window.MTMModules.get(id).shortcut).map(id => [id, async () => {
      const {route, selector} = window.MTMModules.get(id).shortcut;
    ...
    caregiverBabysitter: openBabysitterCareSheet,
    caregiverEmergencyContacts: openEmergencyContacts,
    caregiverEncouragement: openWeeklyEncouragement,
    caregiverTerms: openTermsGuide,
    caregiverSigns: openAutismSignsGuide,
    caregiverAggression: openAggressionGuide,
    caregiverRegulationGuides: openCaregiverRegulationGuides,
    caregiverCalendar: openCaregiverCalendar,
    caregiverTodos: () => openTodoList("active"),
    caregiverReflections: openCaregiverReflections
  };
}

async function renderHome() {
  applyRouteChrome("home");
  view.innerHTML = `<section class="illustrated-home preserved-home" aria-label="More than Measured home">
    <picture><source media="(min-width:700px)" srcset="assets/home/homepage-desktop.webp" type="image/webp"><img src="assets/home/homepage.jpeg" alt="More than Measured — celebrating every child’s unique journey" width="864" height="1536"></picture>
    <h1 class="module-visually-hidden">More than Measured</h1>
    <nav class="home-primary-modules" aria-label="Explore More than Measured">${window.MTMModules.sections.home.map((id, homeSlot) => window.MTMModules.render(id, {presentation: 'bubble', homeSlot})).join('')}</nav>
    <button type="button" class="home-hotspot village home-village-search" data-module="village" aria-label="Search The Village"><span>Search The Village</span></button>
  </section>`;
  window.MTMModules.bind(view, {navigate, actions: moduleActions()});
  await window.MTMLayouts.mount({section: 'home', container: view.querySelector('.home-primary-modules'), presentation: 'bubble', navigate, actions: moduleActions()});
...
function renderSpeechBuilding() {
  ...
  <div class="grid">
    ...
    <button class="card-button speech-guide" data-guide="products"><span class="emoji">🛍️</span><strong>Communication products</strong><small>Product guidance and caregiver-supplied shopping links together.</small></button>
  </div>`;
  bindRouteButtons();
...
function openSpeechGuide(kind){
  ...
  if(kind==="products"){openProductGuide("Communication",["Core-word and choice boards","Portable dry-erase boards and communication books","Recordable single-message or sequential buttons","Photo-card supplies and visual-schedule materials","Device cases, straps, stands, keyguards, and screen protection","Switch-access and alternative-access tools recommended after evaluation"],"speech","Communication tools should remain available and should never be used as a reward or taken away as punishment. AAC products should be selected around the user’s communication and access needs, ideally with help from an AAC-experienced speech-language pathologist when available.");return;}
...
const PACI_CHIROPRACTIC_ARTICLES=[
  ["Resolution of Adult Attention Deficit Hyperactivity Disorder Following Chiropractic Care","2018-07-12","https://www.pacichiropractic.com/article/3185.html"],
  ["Autism and Chiropractic","2019-01-14","https://www.pacichiropractic.com/article/3257.html"],
  ["Improvement in ADHD Symptomatology Following Chiropractic","2022-05-12","https://www.pacichiropractic.com/article/3630.html"],
  ["Positive Health Outcomes for a Child Diagnosed with ADHD Following Chiropractic","2024-03-13","https://www.pacichiropractic.com/article/3803.html"],
  ["Resolution of Chronic Headaches, Improved Sociability, Health & Physical Behavior in a Child with Autism, ADHD and Obsessive Compulsive Disorder Following Chiropractic","2019-07-12","https://www.pacichiropractic.com/article/3331.html"],
  ["Improvement in Behavior and Attention in a Young Girl with ADHD Receiving Chiropractic","2019-07-12","https://www.pacichiropractic.com/article/3333.html"],
  ["Improved Behavioral Changes and Quality of Life in an Adult with Autism Following Chiropractic","2021-03-12","https://www.pacichiropractic.com/article/3521.html"],
  ["Autism and Attention-Deficit-Hyperactive Disorder Helped with Chiropractic","2023-12-12","https://www.pacichiropractic.com/article/3779.html"],
  ["Lowering the Bar: Adult ADHD, a Risky Diagnosis?","2016-10-12","https://www.pacichiropractic.com/article/2933.html"],
  ["Improvement in Behavior, Communication & Sociability in a Child with Autism Following Chiropractic","2018-06-14","https://www.pacichiropractic.com/article/3169.html"],
  ["Autistic Behavior, Language Development, Sleeping, Constipation, and Ear Infections Improved Under Chiropractic","2022-12-13","https://www.pacichiropractic.com/article/3690.html"],
  ["Chiropractic Improves Quality of Life in a Child with a Multitude of Heath Issues","2025-09-14","https://www.pacichiropractic.com/article/3949.html"],
  ["Improvement in a Child with Autism Following Chiropractic","2017-08-11","https://www.pacichiropractic.com/article/3055.html"],
  ["Severe Autistic Spectrum Disorder Improved with Chiropractic","2023-02-13","https://www.pacichiropractic.com/article/3704.html"],
  ["Aggression and Hyperactivity in a Boy with Autism Spectrum Disorder Helped with Chiropractic","2023-11-10","https://www.pacichiropractic.com/article/3773.html"],
  ["Resolution of Bedwetting, Constipation, Headaches, Allergies & Cold Hands Under Chiropractic","2020-09-11","https://www.pacichiropractic.com/article/3473.html"],
  ["Study Shows School Aged Children Benefit from Chiropractic Care","2025-03-12","https://www.pacichiropractic.com/article/3901.html"],
  ["Improved Health and Quality of Life in a Child with Autism Spectrum Disorder","2018-04-12","https://www.pacichiropractic.com/article/3147.html"],
  ["Improvements in Autism Spectrum Disorder Following Chiropractic Care","2018-12-12","https://www.pacichiropractic.com/article/3247.html"],
  ["Autistic Spectrum Disorder Improved with Chiropractic Care","2023-05-11","https://www.pacichiropractic.com/article/3726.html"],
  ["Asperger's Syndrome Symptoms Helped by Chiropractic - A Case Study","2021-04-12","https://www.pacichiropractic.com/article/3533.html"],
  ["Resolution of Systemic Lupus Erythematosus Following Chiropractic Care","2018-01-12","https://www.pacichiropractic.com/article/3115.html"],
  ["Resolution of Asthma and Other Functional Disorders Following Chiropractic","2018-05-10","https://www.pacichiropractic.com/article/3165.html"],
  ["Neck Curvature and Autonomic Nervous Function Improved Under Chiropractic Care","2018-08-13","https://www.pacichiropractic.com/article/3201.html"],
  ["Improvement in ADHD Symptoms in Woman Who Suffered Multiple Physical Traumas","2026-02-15","https://www.pacichiropractic.com/article/3993.html"],
  ["Sleep Teeth Grinding and Clenching Helped in Child Through Chiropractic","2017-11-13","https://www.pacichiropractic.com/article/3087.html"],
  ["Toe Walking and Feeding Difficulties Resolved Following Chiropractic","2018-10-11","https://www.pacichiropractic.com/article/3219.html"],
  ["Quality of Life Improved with Chiropractic for Child Suffering with Asperger's Syndrome","2019-05-13","https://www.pacichiropractic.com/article/3305.html"],
  ["Chiropractic Improves Sleep, Digestion, Behavior, and Allergies in Child with Rare Genetic Disorder","2022-02-14","https://www.pacichiropractic.com/article/3610.html"]
];

function openChiropracticArticleLibrary(){
  const draw=(query="")=>{
    const normalized=query.trim().toLowerCase(),matches=PACI_CHIROPRACTIC_ARTICLES.filter(([title])=>title.toLowerCase().includes(normalized));
    const list=matches.map(([title,date,url])=>`<a class="education-link chiropractic-article-link" href="${url}" target="_blank" rel="noopener"><strong>${esc(title)}</strong><span>Published ${esc(fmtDate(date))}</span><small>Open article on Paci Chiropractic ↗</small></a>`).join("");
...
async function renderHealthWellness(){
  ...
  $("#medicalLetter").onclick=()=>openMedicalNecessityLetter(profiles);$("#apptPrep").onclick=()=>openAppointmentPrep(profiles);$("#providerReport").onclick=()=>openProviderReport(profiles);$("#apptNotes").onclick=()=>openAppointmentNotes(profiles);$("#chiropracticArticles").onclick=openChiropracticArticleLibrary;$("#methylProductsGuide").onclick=openMethylatedProductsGuide;
...
async function renderSleepSanctuary(selectedModule = null) {
  const profiles = await getAll("profiles");
  let profileId = profiles[0]?.id || "";
  let routine = [];
  let preferences = {};
  ...
  if (!profiles.length) { await mountSleepModules(selectedModule); return; }
  ...
  await mountSleepModules(selectedModule);
}

async function mountSleepModules(selectedModule) {
  return mountDetailModules({section: 'sleep', selectedModule, selector: '.sleep-sections > [data-sleep-module]', attribute: 'sleepModule', title: 'Sleep Sanctuary'});
}

async function mountDetailModules({section, selectedModule, selector, attribute, title}) {
  const sections = [...view.querySelectorAll(selector)];
  if (sections.length !== window.MTMModules.sections[section].length) throw new Error('Module content is incomplete.');
  // Move existing form nodes, retaining their listeners, child selectors and
  // saved-record keys. Only navigation changes; no record migration occurs.
  for (const details of sections) {
    const page = document.createElement('section');page.className = 'module-content-page';page.dataset.contentModule = details.dataset[attribute];
    const heading = document.createElement('h2');heading.textContent = window.MTMModules.get(page.dataset.contentModule).title;
    page.append(heading);
    for (const child of [...details.children]) if (child.tagName !== 'SUMMARY') page.append(child);
    page.hidden = page.dataset.contentModule !== selectedModule;details.replaceWith(page);
  }
  const firstPage = view.querySelector('.module-content-page');
  let heading = view.querySelector('.section-title');
  if (!heading) {heading = document.createElement('h2');heading.className = 'section-title';firstPage.before(heading);}
  if (selectedModule) {
    heading.textContent = title;
    const back = document.createElement('button');back.type = 'button';back.className = 'btn secondary';back.textContent = `← ${title}`;back.onclick = () => navigate(section);
    firstPage.before(back);return;
  }
  heading.textContent = section === 'sleep' ? 'Sleep tools' : 'Explore and plan';
  const grid = document.createElement('div');grid.className = 'grid section-grid module-cards';
  grid.innerHTML = window.MTMModules.renderSection(section);firstPage.before(grid);
  window.MTMModules.bind(grid, {navigate});
  await window.MTMLayouts.mount({section, container: grid, presentation: 'card', navigate, actions: moduleActions()});
}
...
async function renderAsdFriendlyFunExpanded(selectedModule = null){
  ...
  const socialization=`<details class="education-card" data-fun-module="funSocial"><summary>🤝 Socialization</summary><div class="education-body">
...
function renderSkills() {
  ...
  $("#skillProducts").onclick=()=>openProductGuide("Skill building",["Visual schedules and first-then boards","Easy-grip utensils and open-cup trainers","Dressing practice boards and adaptive fasteners","Toothbrushing timers and mirrors","Footstools, toilet inserts, and easy clothing","Task boxes, matching sets, and fine-motor tools"],"skill");
}
...
const AMAZON_PRODUCT_CATALOG=[
  ...
  ["speech","Flash cards, third set","https://a.co/d/007Vyp5n"],
  ["speech","Flash cards, fourth set","https://a.co/d/0hWKVUTP"],
  ["speech","Brain Quest cards","https://a.co/d/0cpimNXn"],
  ["speech","Brain Quest cards, second set","https://a.co/d/0hK7y97c"],
  ["speech","Visual communication book","https://a.co/d/0dU5dQsL"],
  ["skill","Emotional skills book","https://a.co/d/0f7ebUF1"],
  ["skill","Visual schedule","https://a.co/d/0cvAjmER"],
  ["speech","Phonics flip book","https://a.co/d/0gVxtXH4"],
  ["safety","Safety product","https://a.co/d/04aSAn6R"],
  ["safety","Safety product, second option","https://a.co/d/05riHf3G"],
  ["safety","Safety product, third option","https://a.co/d/00n0s2dl"],
  ["safety","Safety product, fourth option","https://a.co/d/0hhW21N4"],
  ["safety","Safety product, fifth option","https://a.co/d/0c6v5h49"],
  ["safety","Safety product, sixth option","https://a.co/d/09MTlmrm"],
  ["safety","Safety product, seventh option","https://a.co/d/0iHbbPYz"],
  ["speech","Language and matching activity","https://a.co/d/092RNfT1"],
  ["skill","Busy book","https://a.co/d/05Rx217o"],
  ["skill","Busy book, second option","https://a.co/d/0d6sDkHX"],
  ["skill","Busy book, third option","https://a.co/d/0h2jGEWF"],
  ["sleep","Compression sheet","https://a.co/d/09lzFXNI"],
  ["sleep","Weighted blanket","https://a.co/d/06Z26z2t"],
  ["sleep","Breathing pal","https://a.co/d/0aLzHtcq"],
  ["sleep","Bubble tube","https://a.co/d/0e3hP1ke"],
  ["sleep safety","Bed alarm","https://a.co/d/074ql3if"],
  ["sleep safety","Bed alarm, second option","https://a.co/d/0aVKIxoi"],
...
function openProductGuide(title,items,category,safetyNote=""){
  if(category){openAmazonProductCatalog(category,`${title} products`,safetyNote,`<h3>Product types and shopping guidance</h3><ul>${items.map((x)=>`<li>${esc(x)}</li>`).join("")}</ul><div class="banner">A product should support participation or safety—not force a child to look less autistic.</div>`);return;}
  openInfoGuide(`🛍️ ${title} products`,`<p>These are shopping categories, not endorsements. Choose products around the child’s actual goal, age, size, motor skills, sensory preferences, cleaning needs, supervision, and choking or entrapment risks.</p><ul>${items.map((x)=>`<li>${esc(x)}</li>`).join("")}</ul><p><a href="https://www.amazon.com/s?k=${encodeURIComponent(title+" autism tools")}" target="_blank" rel="noopener">Search products ↗</a></p><div class="banner">A product should support participation or safety—not force a child to look less autistic.</div>`);
}
...
function renderProductDirectory(){
  ...
  draw();
  $("#productDirectorySearch").oninput=(event)=>draw(event.target.value);
}
...
function renderSafetyInformation() {
  ...
  <button id="amazonSafetyProducts" class="card-button" type="button" style="margin-top:16px"><span class="emoji">🛒</span><strong>Safety products</strong><small>Browse alarms, identification, monitoring, and other caregiver-supplied safety options.</small></button>
  ...
  $("#amazonSafetyProducts").onclick=()=>openAmazonProductCatalog("safety","Safety products","A product is only one layer. Check fire-escape access, entrapment, strangulation, battery, choking, privacy, cellular coverage, false-alarm, and emergency-release risks. Alarms and trackers do not replace supervision or calling 911 when a vulnerable child is missing.");
}
...
async function renderCaregiver() {
  ...
  view.innerHTML = `<section class="hero"><h1>💛 Caregiver Corner</h1><p>Support, organization, and clear information for the caregiver.</p></section>
    ...
    <div class="grid section-grid module-cards">${window.MTMModules.renderSection('caregiver', {descriptions: {
      caregiverCalendar: `${upcoming} upcoming ${upcoming === 1 ? "appointment" : "appointments"}.`,
      caregiverTodos: `${activeTodos} active ${activeTodos === 1 ? "task" : "tasks"}.`,
      caregiverReflections: `${reflectionCount} saved ${reflectionCount === 1 ? "entry" : "entries"} • searchable personal journal.`
    }})}</div>`;
  window.MTMModules.bind(view, {navigate, actions: moduleActions()});
  await window.MTMLayouts.mount({section: 'caregiver', container: view.querySelector('.module-cards'), presentation: 'card', navigate, actions: moduleActions(), descriptions: {
    caregiverCalendar: `${upcoming} upcoming ${upcoming === 1 ? "appointment" : "appointments"}.`,
    caregiverTodos: `${activeTodos} active ${activeTodos === 1 ? "task" : "tasks"}.`,
    caregiverReflections: `${reflectionCount} saved ${reflectionCount === 1 ? "entry" : "entries"} • searchable personal journal.`
  }});
...
async function renderSettings() {
  ...
  if (accessStatus.canWrite && accessStatus.role !== 'babysitter') {
    const layoutSettings = document.createElement('section');layoutSettings.className = 'card settings-card';
    layoutSettings.innerHTML = '<h3>Home screen</h3><p>Choose and arrange your eight home shortcuts.</p><button id="customizeHomeLayout" class="btn secondary" type="button">Customize home screen</button>';
    view.querySelector('.hero').after(layoutSettings);
    layoutSettings.querySelector('button').onclick = () => {window.MTMLayouts.requestEditor('home');navigate('home');};
  }
...
function setupDrawer() {
  const links = [
    ...
    ["🌱", "Growth Journey", "child"],
    ["🫧", "My Day", "myDay"],
    ["🗣️", "Speech/Language Building", "speech"],
    ["🌙", "Sleep Sanctuary", "sleep"],
    ["🫧", "Sensory Support", "sensory"],
    ["🩺", "Health and Wellness", "health"],
    ["📚", "Skill Building", "skills"],
    ["📚", "Resources", "resources"],
    ["🛍️", "Product Links", "products"],
