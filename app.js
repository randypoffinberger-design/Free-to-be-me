...
const APP={name:'More than Measured',version:'0.3.0',schemaVersion:1};
...
async function normalizeVocabulary(words){for(const item of words){let changed=false;if(!item.category){item.category='Uncategorized';changed=true;}for(const key of ['speak','identify','asl'])if(item[key]===undefined){item[key]=key==='speak';changed=true;}if(changed)await put('words',item);}return words;}
...
async function renderVocabulary(){
  ...
  <div class="vocab-controls card">
    ...
    <div class="field"><label>Search words, notes, or categories</label><input id="vocabSearch" type="search" placeholder="Search speech and language"></div>
  ...
  const refresh=()=>{
    const profile=$('#vocabProfile').value,q=wordKey($('#vocabSearch').value),category=$('#vocabCategory').value,year=$('#vocabYear').value,month=$('#vocabMonth').value,exact=$('#vocabDate').value,sort=$('#vocabSort').value;
    ...
    let shown=words.filter(x=>(profile==='all'||x.profileId===profile)&&(!category||x.category===category)&&(!q||wordKey(`${x.word} ${x.notes||''} ${x.category||''}`).includes(q))&&(!exact||x.date===exact)&&(!exact&&!year||String(x.date||'').startsWith(year))&&(!exact&&!month||String(x.date||'').slice(5,7)===month));
    ...
    $('#vocabList').innerHTML=shown.length?shown.map(x=>`<details class="word-card card"><summary><strong>${esc(x.word)}</strong>${x.date?`<span>${fmtDate(x.date)}</span>`:''}</summary><div class="word-details"><div class="word-detail-meta"><span class="category-chip">${esc(x.category)}</span><span>${esc(names[x.profileId]||'Child')}</span></div>${x.notes?`<p class="word-notes">${esc(x.notes)}</p>`:''}<div class="ability-checks"><label><input type="checkbox" class="ability-toggle" data-id="${x.id}" data-key="speak" ${capabilityValue(x,'speak')?'checked':''}> Speak</label><label><input type="checkbox" class="ability-toggle" data-id="${x.id}" data-key="identify" ${capabilityValue(x,'identify')?'checked':''}> Identify</label><label><input type="checkbox" class="ability-toggle" data-id="${x.id}" data-key="asl" ${capabilityValue(x,'asl')?'checked':''}> ASL</label></div><div class="word-actions"><button class="small-action edit-word" data-id="${x.id}" type="button">Edit</button><button class="small-action danger-link delete-word" data-id="${x.id}" type="button">Delete</button></div></div></details>`).join(''):`<div class="empty card"><div class="big">🔎</div><p>No speech or language entries match these filters.</p></div>`;
    document.querySelectorAll('.ability-toggle').forEach(box=>box.onchange=async()=>{const item=words.find(x=>x.id===box.dataset.id);if(!item)return;item[box.dataset.key]=box.checked;item.updatedAt=nowISO();await put('words',item);const statId={speak:'totalSpeak',identify:'totalIdentify',asl:'totalAsl'}[box.dataset.key],stat=$('#'+statId);if(stat)stat.textContent=String(Math.max(0,Number(stat.textContent)+(box.checked?1:-1)));});
  ...
  ['vocabProfile','vocabSearch','vocabCategory','vocabSort','vocabYear','vocabMonth','vocabDate'].forEach(id=>$('#'+id).addEventListener(id==='vocabSearch'?'input':'change',refresh));
  $('#clearVocabFilters').onclick=()=>{['vocabCategory','vocabYear','vocabMonth','vocabDate','vocabSearch'].forEach(id=>$('#'+id).value='');$('#vocabProfile').value='all';$('#vocabSort').value='alpha';refresh();};
...
function openWordForm(profiles,item=null,categories=[]){
  modalBody.innerHTML=`<h2>${item?'Edit':'Add'} speech/language entry</h2><div class="form-grid"><div class="field"><label>Child</label><select id="wordProfile">${profiles.map(p=>`<option value="${p.id}" ${item?.profileId===p.id?'selected':''}>${esc(p.name)}</option>`).join('')}</select></div><div class="field"><label>Word or phrase</label><input id="wordText" value="${esc(item?.word||'')}" autocomplete="off"></div><div class="field"><label>Category</label><div class="inline-field"><select id="wordCategory">${categories.map(c=>`<option ${item?.category===c?'selected':''}>${esc(c)}</option>`).join('')}</select><button id="quickCategory" class="btn secondary" type="button">New</button></div></div><div class="field"><label>Date first recorded</label><input id="wordDate" type="date" value="${item?.date||isoToday()}"></div><fieldset class="ability-field"><legend>Current abilities</legend><label><input id="wordSpeak" type="checkbox" ${item?capabilityValue(item,'speak')?'checked':'':'checked'}> Speak</label><label><input id="wordIdentify" type="checkbox" ${item&&capabilityValue(item,'identify')?'checked':''}> Identify</label><label><input id="wordAsl" type="checkbox" ${item&&capabilityValue(item,'asl')?'checked':''}> ASL</label></fieldset><div class="field"><label>Notes <span class="hint">(optional)</span></label><textarea id="wordNotes">${esc(item?.notes||'')}</textarea></div><button id="saveWord" class="btn full" type="button">Save entry</button></div>`;
  ...
  $('#saveWord').onclick=async()=>{const word=$('#wordText').value.trim(),date=$('#wordDate').value,profileId=$('#wordProfile').value;if(!word||!date)return alert('Please enter a word or phrase and a date.');const all=await getAll('words');if(all.some(x=>x.id!==item?.id&&x.profileId===profileId&&wordKey(x.word)===wordKey(word)))return alert('That word or phrase is already listed for this child.');await put('words',{id:item?.id||uid(),profileId,word,date,category:$('#wordCategory').value||'Uncategorized',speak:$('#wordSpeak').checked,identify:$('#wordIdentify').checked,asl:$('#wordAsl').checked,notes:$('#wordNotes').value.trim(),createdAt:item?.createdAt||nowISO(),updatedAt:nowISO(),syncStatus:'local'});modal.close();renderVocabulary();};
}
