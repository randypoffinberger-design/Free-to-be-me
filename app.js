...
const APP = { name: "More than Measured", version: "0.8.23", schemaVersion: 3 };
...
async function renderChild() {
  ...
  $("#providerSummary").onclick = () => openProviderSummary(p);
  bindRouteButtons();
...
async function renderHealthWellness(){
  ...
  $("#medicalLetter").onclick=()=>openMedicalNecessityLetter(profiles);$("#apptPrep").onclick=()=>openAppointmentPrep(profiles);$("#providerReport").onclick=()=>openProviderSummary(profiles);$("#apptNotes").onclick=()=>openAppointmentNotes(profiles);
...
async function openProviderSummary(profiles, initialProfileId = profiles[0]?.id) {
  if (!profiles.length) return alert("Create a child profile first.");
  const ninetyDaysAgo = new Date();
  ninetyDaysAgo.setDate(ninetyDaysAgo.getDate() - 90);
  const defaultFrom = ninetyDaysAgo.toISOString().slice(0, 10);
  const sections = [
    ["profile", "Profile & priorities"],
    ...
    ["sleep", "Sleep & sensory"],
    ["toileting", "Toileting"],
    ["appointments", "Appointment preparation & notes"],
  ];
  modalBody.innerHTML = `<h2>📊 Provider summary</h2><p class="hint">Create an editable report for a doctor, therapist, school team, or other provider. Communication totals show the complete saved history; dated progress uses the selected reporting period.</p><div class="form-grid"><div class="field"><label>Child</label><select id="providerSummaryProfile">${profiles.map((p) => `<option value="${p.id}" ${p.id === initialProfileId ? "selected" : ""}>${esc(p.name)}</option>`).join("")}</select></div><div class="form-grid two-col"><div class="field"><label>From</label><input id="providerSummaryFrom" type="date" value="${defaultFrom}"></div><div class="field"><label>To</label><input id="providerSummaryTo" type="date" value="${isoToday()}"></div></div><fieldset class="filter-abilities"><legend>Include in report</legend>${sections.map(([key, label]) => `<label class="check-option"><input type="checkbox" data-provider-section="${key}" checked> ${label}</label>`).join("")}</fieldset><button id="generateProviderSummary" class="btn full" type="button">Generate or refresh summary</button><div class="field"><label>Review and edit before sharing</label><textarea id="providerSummaryText" class="template-letter" placeholder="Choose the report options, then generate the summary."></textarea></div><div class="btn-row"><button id="shareProviderSummary" class="btn" type="button">Share</button><button id="copyProviderSummary" class="btn secondary" type="button">Copy</button><button id="downloadProviderSummary" class="btn secondary" type="button">Download .txt</button></div><p class="hint">This report can contain sensitive health and developmental information. Review it for accuracy and share it only with someone you trust.</p></div>`;
  modal.showModal();

  const withinRange = (date, from, to) => Boolean(date && date >= from && date <= to);
  const line = (label, value) => `${label}: ${value || "Not entered"}`;
  const listOrNone = (items) => items.length ? items.join("\n") : "None entered";
  const limitedList = (items, limit) => {
    const shown = items.slice(0, limit);
    return `${listOrNone(shown)}${items.length > limit ? `\n- …and ${items.length - limit} more in this period` : ""}`;
  };
  const makeReport = async () => {
    const profile = profiles.find((p) => p.id === $("#providerSummaryProfile").value);
    const from = $("#providerSummaryFrom").value;
    const to = $("#providerSummaryTo").value;
    if (!from || !to) return alert("Choose both a start and end date.");
    if (from > to) return alert("The start date must be before the end date.");
    const included = new Set([...document.querySelectorAll("[data-provider-section]:checked")].map((x) => x.dataset.providerSection));
    if (!included.size) return alert("Choose at least one section to include.");
    const [allWords, allWins, allPotty, food, skills, learning, sleep, routine, materials, prep, appointmentNotes] = await Promise.all([
      getAll("words"), getAll("achievements"), getAll("pottyLogs"), getSetting(`foodDiary:${profile.id}`, []),
      getSetting(`lifeSkills:${profile.id}`, []), getSetting(`learningSnapshot:${profile.id}`, {}),
      getSetting(`sleep:preferences:${profile.id}`, {}), getSetting(`sleep:routine:${profile.id}`, []),
      getSetting(`materialPreferences:${profile.id}`, {}), getSetting(`appointmentPrep:${profile.id}`, {}),
      getSetting(`appointmentNotes:${profile.id}`, []),
    ]);
    const words = allWords.filter((x) => x.profileId === profile.id);
    const wins = allWins.filter((x) => x.profileId === profile.id);
    const potty = allPotty.filter((x) => x.profileId === profile.id);
    const report = [
      "MORE THAN MEASURED™ — CAREGIVER-GENERATED PROVIDER SUMMARY",
      `Generated: ${new Date().toLocaleString()}`,
      `Child: ${profile.name}`,
      `Reporting period: ${fmtDate(from)} through ${fmtDate(to)}`,
    ];
    if (included.has("profile")) report.push("PROFILE & PRIORITIES", line("Date of birth", profile.birthDate ? fmtDate(profile.birthDate) : ""), line("Current age", profile.birthDate ? profileDetail(profile, "yearsMonths") : ""), line("Special interests", profile.specialInterest), line("Currently working on", profile.currentFocus), line("Strengths", learning.strengths), line("Struggles or barriers", learning.struggles), line("What helps", learning.helps));
    if (included.has("communication")) {
      const byType = (type) => words.filter((x) => (x.entryType || "word") === type);
      const totalLine = (label, type, allowIdentify = true) => { const items = byType(type); return `${label}: ${items.length} total; ${items.filter((x) => capabilityValue(x, "speak")).length} say${allowIdentify ? `; ${items.filter((x) => capabilityValue(x, "identify")).length} identify` : ""}; ${items.filter((x) => capabilityValue(x, "asl")).length} ASL`; };
      const recent = [...words].filter((x) => withinRange(entryDate(x), from, to)).sort((a, b) => entryDate(b).localeCompare(entryDate(a))).map((x) => `- ${fmtDate(entryDate(x))}: ${x.word || x.text || x.title} (${x.entryType || "word"})`);
      report.push("COMMUNICATION", totalLine("Words", "word"), totalLine("Sentences", "sentence", false), totalLine("Letters", "letter"), totalLine("Numbers", "number"), `Recent additions (${recent.length}):`, limitedList(recent, 30));
    }
    if (included.has("wins")) {
      const recentWins = [...wins].filter((x) => withinRange(x.date, from, to)).sort((a, b) => (b.date || "").localeCompare(a.date || "")).map((x) => `- ${fmtDate(x.date)}: ${x.title}${x.notes ? ` — ${x.notes}` : ""}`);
      report.push("WINS", limitedList(recentWins, 20));
    }
    if (included.has("skills")) report.push("LIFE SKILLS", listOrNone(skills.map((x) => `- ${x.name}: ${x.status || "Not started"}${x.date ? ` (${fmtDate(x.date)})` : ""}${x.notes ? ` — ${x.notes}` : ""}`)));
    if (included.has("food")) {
      const foodNames = (category) => food.filter((x) => x.category === category).map((x) => x.name).join(", ") || "None entered";
      ...
      report.push("FOOD & REACTIONS", line("Safe", foodNames("safe")), line("Occasionally eats", foodNames("sometimes")), line("Not accepted / do not offer", foodNames("not")), "Allergies, reactions, and sensitivities:", listOrNone(reactions));
    }
    if (included.has("sleep")) {
      const routineText = routine.map((x, i) => `${i + 1}. ${x.time ? `${x.time} — ` : ""}${x.text}`);
      report.push("SLEEP & SENSORY", line("Temperature", sleep.temperature), line("Pressure or compression", sleep.pressure), line("Sleep texture", sleep.texture), line("Light", sleep.light), line("Sound", sleep.sound), line("Movement before bed", sleep.movement), line("Sleep observations", sleep.notes), "Bedtime routine:", listOrNone(routineText), line("Comfortable clothing", materials.comfortableClothing), line("Difficult clothing", materials.difficultClothing), line("Seams, tags, fit, and fasteners", materials.clothingDetails), line("Preferred bedding", materials.preferredBedding), line("Bedding to avoid", materials.avoidBedding), line("Other material notes", materials.notes));
    }
    if (included.has("toileting")) {
      const recentPotty = [...potty].filter((x) => withinRange(x.date, from, to)).sort((a, b) => (b.date || "").localeCompare(a.date || ""));
      const sum = (key) => recentPotty.reduce((n, x) => n + Number(x[key] || 0), 0);
      report.push("TOILETING", `Recorded days: ${recentPotty.length}`, `Pees in potty: ${sum("pees")}`, `Poops in potty: ${sum("poops")}`, `Accidents: ${sum("accidents")}`, limitedList(recentPotty.map((x) => `- ${fmtDate(x.date)}: ${Number(x.pees || 0)} pee, ${Number(x.poops || 0)} poop, ${Number(x.accidents || 0)} accidents${x.notes ? ` — ${x.notes}` : ""}`), 14));
    }
    if (included.has("appointments")) {
      const recentNotes = [...appointmentNotes].filter((x) => withinRange(x.date, from, to)).sort((a, b) => (b.date || "").localeCompare(a.date || "")).map((x) => `- ${fmtDate(x.date)} — ${x.provider || "Appointment"}: ${x.body || "No decisions entered"}${x.follow ? `; Next: ${x.follow}` : ""}`);
      report.push("APPOINTMENT PREPARATION & NOTES", line("Planned visit", prep.visit), line("Top concerns", prep.concerns), line("Changes since last visit", prep.changes), line("Medicines, supplements, and allergies", prep.meds), line("Questions and decisions needed", prep.questions), "Notes from visits in this period:", limitedList(recentNotes, 10));
    }
    report.push("This caregiver-generated summary is not a medical record or diagnosis. Please verify details with the caregiver and the clinical record.", "Prepared with More than Measured™ by Serenity Valley Works.");
    $("#providerSummaryText").value = report.join("\n\n");
  };
  $("#generateProviderSummary").onclick = makeReport;
  $("#copyProviderSummary").onclick = async () => { const text = $("#providerSummaryText").value.trim(); if (!text) return alert("Generate the summary first."); await navigator.clipboard.writeText(text); alert("Provider summary copied."); };
  $("#shareProviderSummary").onclick = async () => { const text = $("#providerSummaryText").value.trim(); if (!text) return alert("Generate the summary first."); if (!navigator.share) { await navigator.clipboard.writeText(text); return alert("Sharing is unavailable here, so the summary was copied instead."); } try { await navigator.share({ title: "Provider summary", text }); } catch (error) { if (error.name !== "AbortError") alert("The summary could not be shared. You can copy or download it instead."); } };
  $("#downloadProviderSummary").onclick = () => { const text = $("#providerSummaryText").value.trim(); if (!text) return alert("Generate the summary first."); const profile = profiles.find((p) => p.id === $("#providerSummaryProfile").value); downloadBlob(new Blob([text], { type: "text/plain" }), `${profile.name.replace(/[^a-z0-9]+/gi, "-")}-Provider-Summary.txt`); };
  await makeReport();
}
